...
mod internal {
    ...
    pub fn query_avfoundation() -> Result<Vec<CameraInfo>, NokhwaError> {
        Ok(AVCaptureDeviceDiscoverySession::new(AVCaptureDeviceType::all())?
        .devices())
    }
    ...
    impl AVCaptureDeviceType {
        ...
        /// are found both before macOS 14 (`ExternalUnknown`) and after
        /// (`External`).
        pub fn all() -> Vec<Self> {
            vec![
                Self::UltraWide,
    ...
    impl AVCaptureDeviceDiscoverySession {
        ...
        pub fn default() -> Result<Self, NokhwaError> {
            AVCaptureDeviceDiscoverySession::new(AVCaptureDeviceType::all())
        }
                    ...
                    && dimensions.width == descriptor.resolution().width() as i32
                {
                    for range in ns_arr_to_vec::<AVFrameRateRange>(unsafe {
                        ...
                        if (f64::from(descriptor.frame_rate()) - max_fps).abs() < 0.999 {
                            ...
                            // with 1080p at both 30 and 5 fps got the 5 fps format
                            // with a 30 fps frame duration, which AVFoundation
                            // rejects with an uncaught exception.
